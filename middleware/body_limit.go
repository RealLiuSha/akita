...
func BodyLimitWithConfig(config BodyLimitConfig) akita.MiddlewareFunc {
	...
	if err != nil {
		panic(fmt.Errorf("akita: invalid body-limit=%s", config.Limit))
	}
	config.limit = limit
	...
	return func(next akita.HandlerFunc) akita.HandlerFunc {
		return func(ctx akita.Context) error {
			...
			if req.ContentLength > config.limit {
				return akita.ErrStatusRequestEntityTooLarge
			}

			// Based on content read
			r := pool.Get().(*limitedReader)
			r.Reset(req.Body, ctx)
			defer pool.Put(r)
			req.Body = r
...
func (r *limitedReader) Read(b []byte) (n int, err error) {
	...
	r.read += int64(n)
	if r.read > r.limit {
		return n, akita.ErrStatusRequestEntityTooLarge
	}
	return
}
...
func (r *limitedReader) Reset(reader io.ReadCloser, context akita.Context) {
	r.reader = reader
	r.context = context
}
