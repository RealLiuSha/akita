...
func TestJWTRace(t *testing.T) {
	...
	makeReq := func(token string) akita.Context {
		req := httptest.NewRequest(akita.GET, "/", nil)
		res := httptest.NewRecorder()
		req.Header.Set(akita.HeaderAuthorization, DefaultJWTConfig.AuthScheme+" "+token)
...
func TestJWT(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		req := httptest.NewRequest(akita.GET, tc.reqURL, nil)
		res := httptest.NewRecorder()
		req.Header.Set(akita.HeaderAuthorization, tc.hdrAuth)
		req.Header.Set(akita.HeaderCookie, tc.hdrCookie)
		ctx := a.NewContext(req, res)
		...
		h := JWTWithConfig(tc.config)(handler)
		if assert.NoError(t, h(ctx), tc.info) {
